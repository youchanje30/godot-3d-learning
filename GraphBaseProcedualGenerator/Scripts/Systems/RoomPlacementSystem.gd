	...
	for angle in angles:
		...
		if collision_system.can_place_room(rotated_positions, target_pos):
			...
			for door_pos in doors:
				...
				grid_system.add_special_cell(rotated_door_pos + target_pos, door_direction)
...
func try_place_random_room(room: Room) -> bool:
	...
	if direction == Vector3i.BACK: preferred_angle = 0
	elif direction == Vector3i.RIGHT: preferred_angle = 1
	elif direction == Vector3i.FORWARD: preferred_angle = 2
	elif direction == Vector3i.LEFT: preferred_angle = 3
